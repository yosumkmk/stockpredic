...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan  4 22:12:05 2018
...
def Dipssasi(stdatas):
    ...
    sig1=np.concatenate([sigonec,sigzeroc,sigonec,n010(stdatas[:,3,:]*0.98),sigzeroc,n010(stdatas[:,3,:]*0.98),ave7h],axis=1)
    ...
    sig=np.where(((np.tile(n011(stc5(sig[:,0,:])),(1,7,stdatas.shape[2]))>=2)&
                 (volave>50))
                ,sig,sigz)
...
def Dipssasisell(stdatas,buysk):
    ...
    for i in range(len(hoji[0,:])):
        ...
        sellsig=np.where((((np.tile(n01(holddays),(1,5)))>2)&
                         ((np.tile(n01(holddays),(1,5)))<=5)&
                         ((np.tile(n01(stdatas[hoji[0,i]::,4,hoji[2,i]]),(1,5))>(np.ones((zdate[i],5))*(stdatas[hoji[0,i]-1,4,hoji[2,i]])*1.2))))
                        ,sellsiga,sellsig)
