...
def Dips31sasi(stdatas):
    ...
    #前日比率が-３.５より大きいとき
    sig1=np.concatenate([sigonec,sigzeroc,sigonec,n010(stdatas[:,4,:]*0.94),sigzeroc,n010(stdatas[:,4,:]*0.965),ave7h],axis=1)
    sig=np.concatenate([sigzeroc,sigzeroc,sigzeroc,sigzeroc,sigzeroc,sigzeroc,sigzeroc],axis=1)
    #stopLはstdatas[:,5,:],stophはstdatas[:,6,:]
    #１→始値、２→高値、３→安値、４→終値
    sig=np.where((((np.tile(n010(hiritu[:,4,:]),(1,7,1)))>-3.5)&
    ...
    sig=np.where(((np.tile(n011(stc5(sig[:,0,:])),(1,7,stdatas.shape[2]))>=3)&
                 (volave>50))
                ,sig,sigz)
...
def Dips31sasisell(stdatas,buysk):
