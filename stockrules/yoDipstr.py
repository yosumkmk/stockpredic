...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan  4 22:17:12 2018
...
def Dipstr(stdatas):
    ...
    maxperiod=[21]
    #nosigiが0なら欠損値なし
    ave21=stmean(stdatas[:,4,:],[7])
    nosig=np.tile(n010(stc3(ave21,maxperiod)),(1,7,1))+np.tile(n010(stc3(stdatas[:,4,:],[20])),(1,7,1))
    #[sig,買い売り、成り行き指値、指値価格、逆指値価格]
    sig1=np.concatenate([sigonec,sigzeroc,sigzeroc,sigzeroc,sigzeroc,n010(stdatas[:,4,:]),ave7h],axis=1)
    sig=np.concatenate([sigzeroc,sigzeroc,sigzeroc,sigzeroc,sigzeroc,sigzeroc,sigzeroc],axis=1)
    #stopLはstdatas[:,5,:],stophはstdatas[:,6,:],呼値は７、出来高/単元株数は８
    #１→始値、２→高値、３→安値、４→終値
    sig=np.where(((ave21h<-15)&
                 (ave15h<-16)&
                 (np.tile(n010(stdatas[:,4,:]),(1,7,1))<stBlower7)&
                 (stac<22)&
                 (volave>0)&
                 (nosig==0))
                ,sig1,sig)
    sigz=np.concatenate([sigzeroc,sigzeroc,sigzeroc,sigzeroc,sigzeroc,sigzeroc,sigzeroc],axis=1)        
    sig=np.where(((np.tile(n011(stc5(sig[:,0,:])),(1,7,stdatas.shape[2]))>=5)&
                 (volave>50))
                ,sig,sigz)
    return sig


def Dipstrsell(stdatas,buysignal):
    ...
    sigm1=deepcopy(buysignal)
    #hoji[(日付、種類、コード）、（シグナル執行順＝合計が買い回数)）
    hoji=deepcopy(np.array(np.where(n010(buysignal[:,0,:])==sigoc)))
    #残り日数
    ...
    for i in range(len(hoji[0,:])):
        ...
        sellsig=np.where(np.tile(n01(holddays),(1,5))>2
                        ,sellsiga,sellsig)
